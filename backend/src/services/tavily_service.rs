...
impl TavilyService {
    ...
    pub async fn search_competitors(
        ...
        info!("Searching Tavily for competitors: {} in {}", query, location);
        
        if self.api_key.is_empty() {
            anyhow::bail!("Tavily API key not configured");
        }

        let search_query = format!("{} competitors {} Saudi Arabia reviews pricing", query, location);
        ...
        info!("Tavily search returned {} results", result["results"].as_array().map(|r| r.len()).unwrap_or(0));
    ...
    pub async fn search_competitor_reviews(
        ...
        let query = format!("{} {} reviews ratings customer feedback Saudi Arabia", business_name, location);
